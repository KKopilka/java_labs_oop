 ...
 * задач искателя, введенных с потоками для их запуска. Затем, когда
 * сканирование завершено, распечатывается список найденных URL-адресов.
 */
...
public class Crawler {
            ...
            if (currentPair.depth < maxDepth) {
                ...
                try {
                    ...
                    while ((line = in.readLine()) != null) {
                        if (line.indexOf(URLDepthPair.URL_PREFIX) != -1 && line.indexOf('"') != -1) {
                            ...
                            /* Добавляем новую ссылку и глубину, если ссылка подходит. */
                            URLDepthPair newPair = new URLDepthPair(currentLink.toString(), currentPair.depth + 1);
                            if (URLDepthPair.check(findLink, newPair) && URLDepthPair.check(viewedLink, newPair)
    ...
    public static void main(String[] args) {
        String[] arg = new String[] { "http://www.nytimes.com", "1" };
        try {
